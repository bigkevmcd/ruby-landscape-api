...
describe Landscape::Client do
  ...
  describe '#add_access_groups_to_role' do
    it 'should request adding an access group to a role' do
      landscape.stub!(:fetch_response).with(
        ...
        params: {'name' => 'MyRole',
                 'access_groups.1' => 'my-group'}).and_return(
          {'key' => 1012,
           'name' => 'MyRole',
           'description' => '',
           'permissions' => [],
           'persons' => [],
           'access_groups' => ['my-group']})
    ...
    it 'should request adding multiple groups to a role' do
      landscape.stub!(:fetch_response).with(
        ...
        params: {'name' => 'MyRole',
                 ...
                 'access_groups.2' => 'my-other-group'}).and_return(
          {'key' => 1012,
           'name' => 'MyRole',
           'description' => '', 
           'permissions' => [],
           'persons' => [],
           'access_groups' => ['my-group', 'my-other-group']})

      landscape.add_access_groups_to_role(
        'MyRole', ['my-group', 'my-other-group'])
    end

    it 'should raise an error on an unknown role' do
      landscape.stub!(:make_request).with(
      ...
      expect {
        landscape.add_access_groups_to_role('NonexistentRole', ['my-group'])
      }.to raise_error(Landscape::Errors::UnknownRole)
    end

    it 'should raise an error on an unknown access group' do
      landscape.stub!(:make_request).with('AddAccessGroupsToRole', {
        'name' => 'MyRole',
        'access_groups.1' => 'test1'}).and_return(
          stub(:body => '{"message": "Unknown access groups for account \'testing\': test1", "error": "UnknownAccessGroups"}'))

      expect {
        landscape.add_access_groups_to_role('MyRole', ['test1'])
      }.to raise_error(Landscape::Errors::UnknownAccessGroups)
    end

    it 'should raise an error when an access group is read-only' do
      landscape.stub!(:make_request).with('AddAccessGroupsToRole', {
        'name' => 'MyRole',
        'access_groups.1' => 'testa'}).and_return(
          stub(:body => '{"message": "The role \'MyRole\' is read-only", "error": "ReadOnlyRole"}'))

      expect {
        landscape.add_access_groups_to_role('MyRole', ['testa'])
      }.to raise_error(Landscape::Errors::ReadOnlyRole)
    end
  end
  ...
  describe '#add_permissions_to_role' do
    ...
    it 'should request adding multiple permissions to a role' do
      landscape.stub!(:fetch_response).with(
        ...
        params: {'name' => 'MyRole',
                 ...
                 'permissions.2' => 'ViewComputer'}).and_return(
          {'key' => 1012,
           'name' => 'MyRole',
           'description' => '',
           'permissions' => ['ExecuteScript', 'ViewComputer'],
           'persons' => [], 'access_groups' => ['my-group']})
    ...
    it 'should raise an error on an unknown role' do
      landscape.stub!(:make_request).with(
        'AddPermissionsToRole', {
          'name' => 'NonexistentRole',
          'permissions.1' => 'my-group'}).and_return(
            stub(:body => '{"message": "Unknown role name: NonexistentRole", "error": "UnknownRole"}'))

      expect {
        landscape.add_permissions_to_role('NonexistentRole', ['my-group'])
      }.to raise_error(Landscape::Errors::UnknownRole)
    end
    ...
    it 'should raise an error on an unknown permission' do
      landscape.stub!(:make_request).with(
        'AddPermissionsToRole', {
          'name' => 'MyRole', 
          'permissions.1' => 'test1'}).and_return(
            stub(:body => '{"message": "Invalid permissions: test1", "error": "InvalidPermissions"}'))

      expect {
        landscape.add_permissions_to_role('MyRole', ['test1'])
      }.to raise_error(Landscape::Errors::InvalidPermissions)
    end
    ...
    it 'should raise an error when a role is read-only' do
      landscape.stub!(:make_request).with(
        'AddPermissionsToRole', {
          'name' => 'MyRole', 
          'permissions.1' => 'ExecuteScript'}).and_return(
            stub(:body => '{"message": "The role \'MyRole\' is read-only", "error": "ReadOnlyRole"}'))
  ...
  describe '#add_persons_to_role' do
    it 'should request adding a person to a role' do
      landscape.stub!(:fetch_response).with(
        action: 'AddPersonsToRole',
        params: {
          'name' => 'MyRole',
          'persons.1' => 'test@example.com'}).and_return(
            {'key' => 1012,
             'name' => 'MyRole',
             'description' => '',
             'permissions' => [],
             'persons' => ['test@example.com'],
             'access_groups' => ['my-group']})
    ...
    it 'should request adding more than one to a role' do
      landscape.stub!(:fetch_response).with(
        ...
        params: {
          'name' => 'MyRole',
          'persons.1' => 'test@example.com',
          'persons.2' => 'test2@example.com'}).and_return(
            {'key' => 1012,
             'name' => 'MyRole',
             'description' => '',
             'permissions' => [],
             'persons' => ['test@example.com', 'test2@example.com'],
             'access_groups' => ['']})

      landscape.add_persons_to_role(
        'MyRole', ['test@example.com', 'test2@example.com'])
    end
    ...
    it 'should raise an error if an unknown email is known' do
      ...
      expect {
        landscape.add_persons_to_role('MyRole', ['test@example.com'])
      }.to raise_error(Landscape::Errors::UnknownPersons)
    end
  end

  describe '#remove_access_group' do
    it 'should request the removal of the access group' do
      landscape.stub!(:fetch_response).with(
        action: 'RemoveAccessGroup',
        params: {'name' => 'MyAccessGroup'}).and_return(
          {'name' => 'ParentAccessGroup',
           'title' => 'parent',
           'parent' => 'GrandParentAccessGroup',
           'children' => ''})

      landscape.remove_access_group('MyAccessGroup')
    end
    it 'should raise an error if no access group is provided' do
      expect {
        landscape.remove_access_group
      }.to raise_error(ArgumentError)
    end
  end

  describe '#remove_access_groups_from_role' do
    it 'should request the removal of the access groups from the named role' do
      landscape.stub!(:fetch_response).with(
        action: 'RemoveAccessGroupsFromRole',
        params: {
          'name' => 'MyRole',
          'access_groups.1' => 'my-group'}).and_return([
            {'key' => 1012, 
      ...
      landscape.remove_access_groups_from_role('MyRole', ['my-group'])
    end
  end

  describe '#remove_permissions_from_role' do
    it 'should request adding a permission to a role' do
      landscape.stub!(:fetch_response).with(
      ...
      landscape.remove_permissions_from_role('MyRole', ['ExecuteScript'])
    end
    ...
    it 'should request adding multiple permissions to a role' do
      ...
      landscape.remove_permissions_from_role('MyRole', ['ExecuteScript', 'ViewComputer'])
    end

    it 'should raise an error on an unknown role' do
      landscape.stub!(:make_request).with('RemovePermissionsFromRole', {
        'name' => 'NonexistentRole',
        'permissions.1' => 'my-group'}).and_return(
          stub(:body => '{"message": "Unknown role name: NonexistentRole", "error": "UnknownRole"}'))

      expect {
        landscape.remove_permissions_from_role('NonexistentRole', ['my-group'])
      }.to raise_error(Landscape::Errors::UnknownRole)
    end
    ...
    it 'should raise an error on an unknown permission' do
      ...
      expect {
        landscape.remove_permissions_from_role('MyRole', ['test1'])
      }.to raise_error(Landscape::Errors::InvalidPermissions)
    end
  ...
  describe '#create_access_group' do
    ...
    it 'should raise an error if no name is supplied' do
      expect {
        landscape.create_access_group(title: 'Test Group')
      }.to raise_error(ArgumentError)
    end
    ...
    it 'should raise an error if we try to create a group with a duplicate name' do
      ...
      expect {
        landscape.create_access_group(name: 'Testing', title: 'Test Group')
      }.to raise_error(Landscape::Errors::DuplicateAccessGroup)
    end
    it 'should raise an error if the name is invalid' do
      ...
      expect {
        landscape.create_access_group(name: 'testing', title: 'Test Group')
        }.to raise_error(Landscape::Errors::InvalidAccessGroup)
    end
  end
  ...
  describe '#create_role' do
    it 'should request creation of new role with a name' do
      landscape.stub!(:fetch_response).with(
        ...
        params: {
          name: 'Tester',
          description: 'Test Role'}).and_return(
            {'key' => 1012,
             'name' => 'Tester',
    ...
    it 'should raise an error if no name is supplied' do
      expect {
        landscape.create_role(description: 'Test Role')
      }.to raise_error(ArgumentError)
    end
    it 'should not raise an error if no title is supplied' do
      landscape.stub!(:fetch_response).with(
        action: 'CreateRole',
        params: {name: 'Testing'}).and_return(
          {'key' => 1012,
            'name' => 'Tester',
            'description' => '',
            'permissions' => [],
    ...
    it 'should raise an error if we try to create a group with a duplicate name' do
      landscape.stub!(:make_request).with('CreateRole', {
          name: 'Testing'}).and_return(
  ...
  describe '#get_roles' do
    ...
    it 'should filter by name if supplied' do
      landscape.stub!(:fetch_response).with(
        action: 'GetRoles',
        params: {'names.1' => 'MyRole'}).and_return([
          {'key' => 1012,
           'name' => 'MyRole',
           'description' => '',
           'permissions' => [],
    ...
    it 'should filter by multiple names if supplied' do
      landscape.stub!(:fetch_response).with(
        action: 'GetRoles',
        params: {'names.1' => 'Test1',
                 'names.2' => 'Test2'}).and_return([
          {'key' => 1012,
           'name' => 'Test1',
           'description' => '',
           'permissions' => [],
